...
class ReleaseBranchWorkflowTests(unittest.TestCase):
    ...
    def test_publish_core_workflow_owns_release_steps(self) -> None:
        ...
        self.assertIn("git add scripts/spm/current_release.json", workflow)
        self.assertIn("gh release create", workflow)
        self.assertIn("gh release upload", workflow)
        self.assertIn("gh api --method PATCH", workflow)
        self.assertIn("git switch --detach", workflow)
        self.assertNotIn('release_branch="release/${{ steps.publication_plan.outputs.final_package_tag }}"', workflow)
        self.assertNotIn('git ls-remote --exit-code --heads origin "refs/heads/${release_branch}"', workflow)
        self.assertNotIn('git fetch origin "refs/heads/${release_branch}:refs/remotes/origin/${release_branch}"', workflow)
        self.assertNotIn('git show "refs/remotes/origin/${release_branch}:Package.swift"', workflow)
        self.assertNotIn('git show "refs/remotes/origin/${release_branch}:scripts/spm/current_release.json"', workflow)
        ...
        self.assertNotIn('git push origin "HEAD:refs/heads/', workflow)
        self.assertIn("package tag already exists with a different Package.swift", workflow)
        self.assertIn("package tag already exists with a different scripts/spm/current_release.json", workflow)
    ...
    def test_package_doc_describes_main_release_branch_model(self) -> None:
        ...
        self.assertIn("do not update the default branch", document)
        self.assertIn("tag-only", document)
        self.assertIn("immutable generated metadata commits", document)
        self.assertIn("not publish `release/<package_tag>` branches", document)
        ...
        self.assertIn("model: `strong-link`", document)
        self.assertIn("install name: `@rpath/MoltenVK.framework/MoltenVK`", document)
    ...
    def test_publish_core_runs_packaging_verification_gates(self) -> None:
        ...
        self.assertIn("--require-strong-dependency @rpath/MoltenVK.framework/MoltenVK", build_job)
        self.assertIn("--forbid-dependency @rpath/libvulkan.dylib", build_job)
        self.assertIn("--forbid-dependency @rpath/libvulkan.1.dylib", build_job)
        ...
        self.assertIn("id: restore_ccache", build_job)
        self.assertIn("uses: actions/cache/restore@v5", build_job)
        self.assertIn("id: build_xcframework", build_job)
        self.assertIn("id: verify_ccache_payload", build_job)
        self.assertIn("uses: actions/cache/save@v5", build_job)
        self.assertIn("steps.restore_ccache.outcome == 'success'", build_job)
        self.assertIn("steps.build_xcframework.outcome == 'success'", build_job)
        self.assertIn("steps.verify_ccache_payload.outcome == 'success'", build_job)
        self.assertIn("steps.restore_ccache.outputs.cache-hit != 'true'", build_job)
        self.assertIn("steps.restore_ccache.outputs.cache-primary-key", build_job)
        self.assertIn("compiler cache remained empty after build-xcframework", build_job)
        ...
        self.assertIn("scripts/spm/prepare_moltenvk_dependency.py", build_job)
        self.assertIn("scripts/spm/moltenvk_dependency.json", build_job)
        ...
        self.assertIn("Vulkan runtime contract: NCNNVulkan strong-links @rpath/MoltenVK.framework/MoltenVK", release_job)
    ...
    def test_readme_describes_wrapper_repo_only(self) -> None:
        ...
        self.assertIn("Alpha package tags point directly at immutable generated metadata commits", readme)
        self.assertNotIn("Alpha releases publish from `release/", readme)
        self.assertIn("Stable promotions may update the default branch", readme)
