...
def _stage_dynamic_library(source_binary: Path, output_dir: Path, target_name: str, environment: dict[str, str]) -> Path:
...
def _build_platform_slice(
    ...
    _run(["cmake", "--build", str(build_dir), "--config", "Release", "--target", "install"], env=environment)
